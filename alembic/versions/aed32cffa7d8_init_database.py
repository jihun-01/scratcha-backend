...
Revision ID: aed32cffa7d8
Revises:
Create Date: 2025-08-27 00:27:42.360823
...
def upgrade() -> None:
    ...
    op.create_table('captcha_problem',
                    ...
                    sa.Column('wrong_answer_1', sa.String(length=20),
                              nullable=False, comment='오답1'),
                    sa.Column('wrong_answer_2', sa.String(length=20),
                              nullable=False, comment='오답2'),
                    sa.Column('wrong_answer_3', sa.String(length=20),
    ...
    op.create_table('usage_stats',
                    sa.Column('id', sa.Integer(), autoincrement=True,
                              nullable=False, comment='사용량 통계 ID'),
                    sa.Column('api_key_id', sa.Integer(),
                              nullable=False, comment='통계의 기준이되는 API 키'),
                    sa.Column('date', sa.Date(), nullable=False,
                              comment='통계 기준 날짜 (예: 2025-07-01)'),
                    sa.Column('captcha_total_requests', sa.Integer(),
                              nullable=False, comment='전체 요청 수'),
                    sa.Column('captcha_success_count', sa.Integer(),
                              nullable=False, comment='성공 응답 수'),
                    sa.Column('captcha_fail_count', sa.Integer(),
                              nullable=False, comment='실패 응답 수'),
                    sa.Column('captcha_timeout_count', sa.Integer(),
                              nullable=False, comment='타임아웃 응답 수'),
                    sa.Column('total_latency_ms', sa.Integer(),
                              nullable=False, comment='총 지연 시간 (ms)'),
                    sa.Column('verification_count', sa.Integer(),
                              nullable=False, comment='총 검증 횟수'),
                    sa.Column('avg_response_time_ms', sa.Float(),
                              nullable=False, comment='평균 응답 시간 (ms)'),
                    sa.Column('created_at', sa.DateTime(), server_default=sa.text(
                        'now()'), nullable=False, comment='레코드 생성 시각 (기본값으로 현재 시각 사용)'),
                    sa.ForeignKeyConstraint(['api_key_id'], ['api_key.id'], ),
                    sa.PrimaryKeyConstraint('id')
                    )
    op.create_table('captcha_log',
                    ...
                    sa.Column('result', sa.Enum('SUCCESS', 'FAIL', 'TIMEOUT',
                                                name='captcharesult'), nullable=False, comment='성공 / 실패 / 타임아웃'),
                    sa.Column('latency_ms', sa.Integer(), nullable=False,
                              comment='캡챠 문제가 해결되기까지 걸린 시간(밀리초)'),
                    sa.Column('created_at', sa.DateTime(), server_default=sa.text(
                        'now()'), nullable=False, comment='문제 생성 시간'),
                    sa.ForeignKeyConstraint(['api_key_id'], ['api_key.id'], ),
                    sa.ForeignKeyConstraint(
